...
host('beschlussarchiv')
    ->set('remote_user', 'ubuntu')
    ->set('deploy_path', '/var/www/share/beschlussarchiv')
    ->set('identity_file', '~/.ssh/beschlussarchiv')
...
task('build:javascript', function () {
    run('cd {{release_path}} && nvm use 18 && bun install && bun run build');
});

after('artisan:migrate', 'build:javascript');
